...
const ResultsTab = ({ result, specificationItemId, onResultLoaded, onLoadLastResult, validationError }) => {
    ...
    useEffect(() => {
        console.log('ResultsTab: New results received, clearing applied items state');
    ...
    const handleApply = async (item, index) => {
        ...
        try {
            ...
            console.log('Codebeamer Assistant: Description updated, requesting page reload');
            ...
            chrome.runtime.sendMessage({ type: 'reload_codebeamer_page' }).catch((error) => {
                ...
                console.log('Codebeamer Assistant: Page reload requested');
            });
        } catch (error) {
            console.error('Error applying proposal:', error);
            setApplyErrors(prev => ({ ...prev, [index]: error.message || 'Failed to apply proposal' }));
        } finally {
    ...
    const handleFeedback = (item, index, isCorrect) => {
        console.log(`Feedback: ${isCorrect ? 'Correct' : 'Incorrect'} for item:`, index, item);
        ...
        <div className="results-container">
            {validationError ? (
                ...
                <div className="table-container">
                    {result.validation_results && Array.isArray(result.validation_results) ? (
                            ...
                            <tbody>
                                {result.validation_results.map((item, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className={`compliance-status ${item.complianceStatus?.toLowerCase()}`}>
                                        ...
                                        <td className="action-cell">
                                            <div className="action-wrapper">
                                                <button 
                                                    className={`action-btn apply-btn ${appliedItems.has(index) ? 'applied' : ''}`}
                                                    onClick={() => handleApply(item, index)}
                                                    disabled={applyingItems.has(index) || appliedItems.has(index)}
                                                >
                                                    {applyingItems.has(index) ? 'Applying...' : appliedItems.has(index) ? 'Applied ✓' : 'Apply'}
                                                </button>
                                                {applyErrors[index] && (
                                                    <div className="error-message" title={applyErrors[index]}>
                        ...
                        <div className="table-container">
                            <div className="no-validation-results">
                                <p>Invalid result format. Expected validation_results with array of items.</p>
                                <details>
