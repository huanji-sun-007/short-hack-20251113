...
import { validateSpecification, getLastValidationResult } from '../../services/backendApi';
import { initAuth, isAuthenticated, login, logout, getCurrentUser } from '../../services/authService';
import './Sidepanel.css';

const Sidepanel = () => {
    const [pageContext, setPageContext] = useState({ projectId: null, trackerId: null, specificationItemId: null });
    ...
    useEffect(() => {
        ...
        const messageListener = (message, sender, sendResponse) => {
            console.log('Codebeamer Assistant: Received message from background script:', message);
            if (message.type === 'page_context_data') {
                setPageContext(message.data || { projectId: null, trackerId: null, specificationItemId: null });
                setIsLoading(false);
    ...
    const reloadPanel = () => {
        console.log('Codebeamer Assistant: Reloading sidepanel');
        window.location.reload();
    };

    const reloadExtension = () => {
        chrome.runtime.sendMessage({ type: 'reload_service_worker' }).catch((error) => {
            console.error('Error requesting service worker reload:', error);
        }).finally(() => {
            console.log('Codebeamer Assistant: Requested reloading service worker');
        });
        reloadPanel();
    };

    const reloadCodebeamerPage = () => {
        console.log('Codebeamer Assistant: Triggering Codebeamer page reload from sidebar');
        chrome.runtime.sendMessage({ type: 'reload_codebeamer_page' }).catch((error) => {
            console.error('Error requesting Codebeamer page reload:', error);
        });
    };

    const reloadChat = () => {
        console.log('Codebeamer Assistant: Reloading chat window');
    ...
    const handleValidate = async () => {
        ...
        try {
            console.log('Starting validation for specification item:', pageContext.specificationItemId);
            const validationResult = await validateSpecification(pageContext.specificationItemId);
    ...
    const handleLoadLastResult = async () => {
        ...
        try {
            console.log('Loading last validation result for specification item:', pageContext.specificationItemId);
            const cachedResult = await getLastValidationResult(pageContext.specificationItemId);
            ...
            if (cachedResult?.result && cachedResult.result.total_validations > 0) {
                ...
                setValidationError('No previous validation results found for this specification');
            }
        } catch (error) {
            console.error('Failed to load last result:', error);
            setValidationError(error.message || 'Failed to load cached results. Please try again.');
        } finally {
    ...
    const contextCount = Object.values(pageContext).filter(value => value !== null).length;
                        ...
                        {isAuthLoading ? (
                            ...
                            <div className="auth-login">
                                <div className="login-message">
                                    <p>Please login with your Microsoft account to use the Codebeamer Assistant.</p>
                                </div>
    ...
    return (
        <div className="sidepanel-container">
            ...
            <div className="top-section">
                <div className="tab-container">
                    <div className="tab-header">
                        <button 
                            className={`tab-button ${activeTab === 'pageContext' ? 'active' : ''}`}
                                    ...
                                    <>
                                        <table className="itemid-table">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td className="context-label">Specification Item ID</td>
                                                    <td className="context-value">
                            ...
                            <div className="validation-tab-container">
                                <div className="validation-section">
                                    <button 
                                        ...
                                        disabled={!pageContext.specificationItemId || isLoadingLast || isValidating}
                                        title={!pageContext.specificationItemId ? 'No specification item ID available' : 'Load last validation result from cache'}
                                    >
                                        {isLoadingLast && (
                                    ...
                                    <button 
                                        ...
                                        disabled={!pageContext.specificationItemId || isValidating || isLoadingLast}
                                        title={!pageContext.specificationItemId ? 'No specification item ID available' : 'Run new validation against regulations'}
                                    >
                                        {isValidating && (
                                            <span className="loader-spinner"></span>
                                        )}
            ...
            <div className={`chat-section ${isChatVisible ? 'visible' : 'hidden'}`}>
                ...
                >
                    <span className="chat-title">Codebeamer AI Assistant</span>
                    <span className="chat-toggle-icon">
                        {isChatVisible ? '▼' : '▲'}
                    </span>
                </div>
                <div className={`chat-content ${isChatVisible ? 'visible' : 'hidden'}`}>
                    <AgentChat key={chatKey} pageContext={pageContext} result={result} onResultUpdate={handleResultUpdate} />
                </div>
            </div>
            
            <div className="reload-buttons-section">
                <button className="reload-cb-btn" onClick={reloadCodebeamerPage} title="Reload Main">
                    ↻ Main
                </button>
                <button className="reload-panel-btn" onClick={reloadPanel} title="Reload Sidepanel">
                    ↻ Sidepanel
                </button>
                <button className="reload-chat-btn" onClick={reloadChat} title="Reload Chat">
                    ↻ Chat
                </button>
                <button className="reload-extension-btn" onClick={reloadExtension} title="Reload Extension">
                    ↻ Extension
                </button>
            </div>
        </div>
